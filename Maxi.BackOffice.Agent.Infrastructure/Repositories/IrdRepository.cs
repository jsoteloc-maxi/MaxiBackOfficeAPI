...
namespace Maxi.BackOffice.Agent.Infrastructure.Repositories
{
    ...
    public class IrdRepository : IIrdRepository
    {
        ...
        public IrdRepository(UnitOfWorkSqlServerAdapter dbCtx)
        {
            this.db = dbCtx;
            this.session = dbCtx.SessionCtx;
        }
        ...
        public dynamic ObtenerImagenesIRD(int idCheckOri, bool ignoreDeny=false,bool IsIrdPrinted=true)
        {
            ...
            try
            {
                var rejectInfo = db.Conn.QuerySingle<CheckRejectHistoryEntity>(
                ...
                {
                    var s = db.LangResource("IrdWasPrinted", "IRD WAS PRINTED");
                    throw new Exception(s);
                }

                var checkOri = db.GetEntityById<ChecksEntity>(idCheckOri);
                ...
                db.Conn.Execute(sqlUpd, new { IrdMicr=rejectInfo.IrdMicr, Id = rejectInfo.IdCheckRejectHistory, IrdPrinted=IsIrdPrinted }, db.Tran);
        ...
        private dynamic CreateImagesIrd(ChecksEntity checkInfo, CheckRejectHistoryEntity rejectInfo)
        {
            ...
            var rrInfo = db.GetEntityById<CC_ReturnedReasonsEntity>(rejectInfo.IdReturnedReason);
            if (rrInfo == null)
            {
            ...
            //vlRetDate = db.Conn.ExecuteScalar<DateTime>("SELECT GETDATE()").ToString("MM/dd/yyyy");
            ...
            {
                var r = db.GetEntityById<CheckProcessorBankEntity>(checkInfo.IdCheckProcessorBank);
                if (r == null)
            ...
            var datePaid = db.Conn.ExecuteScalar<DateTime>(
            ...
            {
                GLogger.Error(ex);
                var s = db.LangResource("CheckImageNotFound", "Original Check Images not Found");
                throw new Exception(s);
            }
        ...
        private dynamic GenerarIrd_Deprecated(int idCheck)
        {
            ...
            try
            {
                ...
                var checkInfo = db.GetEntityById<ChecksEntity>(idCheck);
                ...
                //var rrInfo = db.Conn.QueryFirstOrDefault<CC_ReturnedReasonsEntity>(
                ...
                {
                    GLogger.Error(ex);
                    var s = db.LangResource("CheckImageNotFound", "Imagen de cheque no encontrada");
                    throw new Exception(s);
                }
                ...
                //x.GetById(db.conn, db.tran);

                var sqlUpd = @"UPDATE dbo.CheckRejectHistory SET IrdPrinted=1 WHERE IdCheckRejectHistory=@Id";
                db.Conn.Execute(sqlUpd, new { Id = rejectInfo.IdCheckRejectHistory }, db.Tran);
        ...
        void GetOriginalCheckImages(int idIssuer, int idCheck, out string fileFront, out string fileRear)
        {
            ...
            try
            {
                var picF = db.Conn.QueryFirstOrDefault(sqlImg, new { IdCheck = idCheck, ImageType = 1 }, db.Tran);
                var picB = db.Conn.QueryFirstOrDefault(sqlImg, new { IdCheck = idCheck, ImageType = 2 }, db.Tran);
                if (picF == null || picB == null)
                    throw new Exception($"Check Images Not Found in Table UploadFiles, IdCheck={idCheck}");

                var path = db.GlobalAttr("ChecksPath");
        ...
        private Dictionary<string, string> GetReportParams(int idCheckOrig, string logoFile)
        {
            ...
            GLogger.Debug($"  GetEntityById<AgentEntity>( {session.IdAgent} )");
            var agent = db.GetEntityById<AgentEntity>(session.IdAgent);
            ...
            var rejectInf = db.GetEntityByFilter<CheckRejectHistoryEntity>(
                "IdCheck=@idCheckOrig",
                new { idCheckOrig }).FirstOrDefault();
            if (rejectInf == null) throw new Exception($"RejectHistory ({idCheckOrig}) not found");

            var reasonInf = db.GetEntityById<CC_ReturnedReasonsEntity>(rejectInf.IdReturnedReason);
            if (reasonInf == null) throw new Exception($"IdReturnedReason ({rejectInf.IdReturnedReason}) not found");

            var checkOrig = db.GetEntityById<ChecksEntity>(idCheckOrig);
            ...
            r.Add("CorpPhone", "");
            r.Add("UserLogin", session.UserName);
